...
function LoadEmploymentApp(){
    $.post('https://mt-phone/GetGroupsApp', JSON.stringify({}), function(Data){
        AddDIV(Data)
    });
}
...
$(document).on('click', '#employment-sbmit-for-create-group', function(e){
    ...
    if (Name != "" && pass != "" && pass2 != ""){
        if(pass == pass2){
            $.post('https://mt-phone/employment_CreateJobGroup', JSON.stringify({
                name: Name,
                pass: pass,
            }));
...
$(document).on('click', '#employment-delete-group', function(e){
    ...
    $.post('https://mt-phone/employment_DeleteGroup', JSON.stringify({
        delete: Delete,
    }));
});
...
$(document).on('click', '#employment-sbmit-for-join-group', function(e){
    ...
    if(EnterPass == JoinPass){
        var CSN = QB.Phone.Data.PlayerData.citizenid;
        $.post('https://mt-phone/employment_JoinTheGroup', JSON.stringify({
            PCSN: CSN,
            id: JoinID,
        }));
        ClearInputNew()
...
$(document).on('click', '#employment-list-group', function(e){
    e.preventDefault();
    var id = $(this).data('id')
    $.post('https://mt-phone/employment_CheckPlayerNames', JSON.stringify({
        id: id,
        }), function(Data){
           ClearInputNew()
...
$(document).on('click', '#employment-leave-grouped', function(e){
    ...
    var CSN = QB.Phone.Data.PlayerData.citizenid;
    var id = $(this).data('id')
    $.post('https://mt-phone/employment_leave_grouped', JSON.stringify({
        id: id,
        csn: CSN,
    }));
});