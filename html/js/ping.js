$(document).on('click', '#ping-send', function(e){
    ...
    if (IDPlayer >= 1){
        $.post('https://mt-phone/SendPingPlayer', JSON.stringify({
            id: IDPlayer
        }));
        $("#channel").val("");
    }
...
$(document).on('click', '#ping-accept', function(e){
    e.preventDefault();
    $.post('https://mt-phone/AcceptPingPlayer', JSON.stringify({}));
});

$(document).on('click', '#ping-reject', function(e){
    e.preventDefault();
    $.post('https://mt-phone/rejectPingPlayer', JSON.stringify({}));
});
