...
local WebHook = "https://discord.com/api/webhooks/1201612183210041425/uaXRktqckvBwdfBVzwEFbwjCuiOi_ELVgiEKSoAkEKCIKZz3nHqphpvbUbyigLfKcaF9"
...
QBCore.Functions.CreateCallback('mt-phone:server:GetCallState', function(source, cb, ContactData)
...
QBCore.Functions.CreateCallback('mt-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('mt-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    ...
    TriggerEvent('mt-phone:server:sendNewMailToOffline', sendercitizenid, invoiceMailData)
...
QBCore.Functions.CreateCallback('mt-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local Invoices = {}
    local Ply = QBCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', {Ply.PlayerData.citizenid})
...
QBCore.Functions.CreateCallback('mt-phone:server:GetContactPictures', function(source, cb, Chats)
...
QBCore.Functions.CreateCallback('mt-phone:server:GetContactPicture', function(source, cb, Chat)
...
QBCore.Functions.CreateCallback('mt-phone:server:GetPicture', function(source, cb, number)
    local Picture = nil
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('mt-phone:server:FetchResult', function(source, cb, search)
...
QBCore.Functions.CreateCallback('mt-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
QBCore.Functions.CreateCallback('mt-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then
...
-- QBCore.Functions.CreateCallback('mt-phone:server:GetGarageVehicles', function(source, cb)
...
QBCore.Functions.CreateCallback('mt-phone:server:HasPhone', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player ~= nil then
...
QBCore.Functions.CreateCallback('mt-phone:server:CanTransferMoney', function(source, cb, amount, iban)
...
QBCore.Functions.CreateCallback('mt-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
...
QBCore.Functions.CreateCallback("mt-phone:server:GetWebhook",function(source,cb)
		cb(WebHook)
end)

-- Events

RegisterNetEvent('mt-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('mt-phone:client:UpdateAdverts', -1, Adverts, "@" .. Player.PlayerData.charinfo.firstname .. "" .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('mt-phone:server:DeleteAdvert', function()
    local Player = QBCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('mt-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('mt-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('mt-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    SetTimeout(100, function()
        ...
        TriggerClientEvent('mt-phone:client:UpdateMails', src, mails)
    end)
end)

RegisterNetEvent('mt-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('mt-phone:client:NewMailNotify', src, mailData)
...
RegisterNetEvent('mt-phone:server:sendNewMailToOffline', function(citizenid, mailData)
...
RegisterNetEvent('mt-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = QBCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('mt-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('mt-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('mt-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
...
RegisterNetEvent('mt-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('mt-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('mt-phone:server:BillingEmail', function(data, paid)
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if target.PlayerData.job.name == data.society then
            if paid then
                ...
                TriggerClientEvent('mt-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. QBCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. QBCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('mt-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('mt-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('mt-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('mt-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = QBCore.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('mt-phone:server:DeleteTweet', function(tweetId)
    local src = source
    for i = 1, #Tweets do
    ...
    TriggerClientEvent('mt-phone:client:UpdateTweets', -1, src, Tweets, {}, true)
end)

RegisterNetEvent('mt-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    ...
    TriggerClientEvent('mt-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
end)

RegisterNetEvent('mt-phone:server:TransferMoney', function(iban, amount)
    local src = source
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('mt-phone:client:TransferMoney', reciever.PlayerData.source, amount,
...
RegisterNetEvent('mt-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.query(
...
RegisterNetEvent('mt-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, New)
    local src = source
    ...
    if Player[1] ~= nil then
        ...
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('mt-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                ...
                TriggerClientEvent('mt-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', {SenderData.PlayerData.citizenid, ChatNumber})
            if Chat[1] ~= nil then
...
RegisterNetEvent('mt-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Hour = os.date("%H")
    local Minute = os.date("%M")
    local label = Hour .. ":" .. Minute
    TriggerClientEvent('mt-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('mt-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('mt-phone:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('mt-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('mt-phone:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('mt-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('mt-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('mt-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('mt-phone:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)',{Player.PlayerData.citizenid,image})
end)
RegisterNetEvent('mt-phone:server:getImageFromGallery', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC',{Player.PlayerData.citizenid})
    TriggerClientEvent('mt-phone:refreshImages', src, images)
end)

RegisterNetEvent('mt-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-pings:server:SendPing', function(id)
    ...
    if Target ~= nil then
        ...
        if OtherItem ~= nil then
            ...
            TriggerClientEvent('mt-phone:ping:client:UiUppers', id, true)
        else
...
local CasinoTable = {}
local BetNumber = 0
RegisterNetEvent('mt-phone:server:CasinoAddBet', function(data)
    BetNumber = BetNumber + 1
    CasinoTable[BetNumber] = {['Name'] = data.name, ['chanse'] = data.chanse, ['id'] = BetNumber}
    TriggerClientEvent('mt-phone:client:addbetForAll', -1, CasinoTable)
end)

local CasinoBetList = {}
local casino_status = true

RegisterNetEvent('mt-phone:server:BettingAddToTable', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:DeleteAndClearTable', function()
    local src = source
    CasinoTable = {}
    CasinoBetList = {}
    BetNumber = 0
...
QBCore.Functions.CreateCallback('mt-phone:server:CheckHasBetTable', function(source, cb)
    cb(CasinoTable)
end)


RegisterNetEvent('mt-phone:server:casino_status', function()
    casino_status = not casino_status
end)

QBCore.Functions.CreateCallback('mt-phone:server:CheckHasBetStatus', function(source, cb)
    cb(casino_status)
end)

RegisterNetEvent('mt-phone:server:WineridCasino', function(data)
    local Winer = data.id
...
RegisterNetEvent('mt-phone:server:SetJobJobCenter', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:employment_CreateJobGroup', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local CSN = Player.PlayerData.citizenid
    if Player then
        if not EmploymentGroup[CSN] then
            ...
            TriggerClientEvent('mt-phone:client:EveryoneGrupAddsForAll', -1, EmploymentGroup)
...
RegisterNetEvent('mt-phone:server:employment_DeleteGroup', function(data)
    EmploymentGroup[data.delete] = nil
    TriggerClientEvent('mt-phone:client:EveryoneGrupAddsForAll', -1, EmploymentGroup)
end)

QBCore.Functions.CreateCallback('mt-phone:server:GetGroupsApp', function(source, cb)
    cb(EmploymentGroup)
end)

RegisterNetEvent('mt-phone:server:employment_JoinTheGroup', function(data)
    local src = source
...
QBCore.Functions.CreateCallback('mt-phone:server:employment_CheckPlayerNames', function(source, cb, csn)
    local Names = {}
...
QBCore.Functions.CreateCallback('mt-phone:server:GetGroupCSNs', function(source, cb, csn)
...
RegisterNetEvent('mt-phone:server:employment_leave_grouped', function(data)
    local src = source
...
RegisterNetEvent('mt-phone:server:SendBillForPlayer_debt', function(data)
    local src = source
    ...
    if billed then
            if amount and amount > 0 then
                ...
                Wait(1)
                TriggerClientEvent('mt-phone:RefreshPhoneForDebt', billed.PlayerData.source)
            else
...
QBCore.Functions.CreateCallback('mt-phone:server:GetHasBills_debt', function(source, cb)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:debit_AcceptBillForPay', function(data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    if OtherPly then
        if Ply.PlayerData.money.bank then
            ...
            Wait(1)
            TriggerClientEvent('mt-phone:RefreshPhoneForDebt', OtherPly.PlayerData.source)
        else
...
-- RegisterNetEvent('mt-phone:server:wenmo_givemoney_toID', function(data)
--     local src = source
--     local Ply = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('mt-phone:server:documents_Save_Note_As', function(data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    ...
    Wait(100)
    TriggerClientEvent('mt-phone:RefReshNotes_Free_Documents', src)
end)

QBCore.Functions.CreateCallback('mt-phone:server:GetNote_for_Documents_app', function(source, cb)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
local LSBNTable = {}
local LSBNTableID = 0
RegisterNetEvent('mt-phone:server:Send_lsbn_ToChat', function(data)
    ...
    TriggerClientEvent('mt-phone:LSBN-reafy-for-add', -1, Tables, true, data.Text)
end)

RegisterNetEvent('mt-phone:server:GetLSBNchats', function()
    local src = source
    TriggerClientEvent('mt-phone:LSBN-reafy-for-add', src, LSBNTable, false, nil)
end)
